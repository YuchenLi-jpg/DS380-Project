...
```{r}
fviz_dend(hc_avg, as.ggplot = TRUE, show_labels = FALSE, main = "Euclidean-Average")
```
```{r}
fviz_nbclust(dfML3, hcut, hc_method ="average", hc_metric = "euclidean", method ="silhouette")
```
...
```{r}
external <- dfML2 %>% select(-c("institution"))
external
```
```{r}
res2 <- external %>% prcomp(scale = TRUE) # scale to have unit variance before analysis
str(res2)
```

```{r}
loadings2 <- res2$rotation
score_mat2 <- res2$x
loadings[ , 1:5]
```
